AWS_PROFILE ?= kompas-dev
UPLOAD_FILE ?= data/CompanyDocuments/invoices/invoice_10248.pdf
CHAT_QUERY ?= What is the total amount of the invoice?

.PHONY: test enable-pgvector
...
test:
	...
	@curl -X POST -H "Content-Type: application/json" -d '{"query": "$(CHAT_QUERY)"}' $(CHAT_API_URL)chat
